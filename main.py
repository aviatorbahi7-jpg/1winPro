...
LANGUAGES = {
    ...
    'bd': {'name': '🇧🇩 Bangla', 'earn_btn': 'টাকা আয় শুরু করুন', 'reg_btn': 'রেজিস্ট্রেশন', 'verify_btn': '✅ ভেরিফাই করুন', 'ask_id': 'আপনার আইডি দিন:', 'success': '✅ ভেরিফাইড!'},
}
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('l_'):
        ...
        await context.bot.send_photo(query.from_user.id, IMAGE_WELCOME, caption="Welcome!", reply_markup=InlineKeyboardMarkup(btn))
        ...
        await query.message.delete()
        await context.bot.send_photo(query.from_user.id, IMAGE_REG, caption="<b>Promo Code: BLACK110</b>", parse_mode='HTML', reply_markup=InlineKeyboardMarkup(btn))
        ...
        btn = [[InlineKeyboardButton("Aviator Hack", web_app=WebAppInfo(url=LINK_AVIATOR))],
               [InlineKeyboardButton("Mines Hack", web_app=WebAppInfo(url=LINK_MINES))]]
        await query.message.reply_text("Select Game:", reply_markup=InlineKeyboardMarkup(btn))
